    ...
    const open_reply = new MutationObserver((entries) => {
        ...
        while (i < replies_box.length) { 
            ...
            if (!reply_data.hasOwnProperty(channel_name)) {
                reply_data[channel_name] = { reply_boxes: [reply_box], replies: [reply], img: img } 
            } else {
                ...
                if (check_spam(reply, replies, reply_box, reply_boxes)) {
                    ...
                    for (let comment_box of reply_boxes) { comment_box.remove(); }
        ...
        while (i >= 0) {
            ...
            if (!comment_data.hasOwnProperty(channel_name)) {
                ...
                if (check_spam(comment, comments, comment_box, comment_boxes)) {
                    console.log(comment_data[channel_name])
                    blacklist.push(channel_name)
